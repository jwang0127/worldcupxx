...
def main() -> None:
    ...
    m102 = {
        ...
        "direction":"英格兰胜，重点防平",
        "probability":"英格兰胜37% / 平33% / 阿根廷胜30%",
        "main_score":"1-1", "goals":"2球主线，防1/3球",
        "advance":"英格兰", "advance_prob":"英格兰52% / 阿根廷48%",
        "upset":"阿根廷0-1、0-2或1-2",
        "hafu":"胜胜 / 平平 / 平胜，防负负",
        "fitness":"两队八强均打加时；阿根廷更晚结束且少休息约4小时，体能项小幅偏英格兰。",
        "analysis":f"竞彩HAD {odds['had']['home']}/{odds['had']['draw']}/{odds['had']['away']}把英格兰放在轻微优势位，归一化约37%/33%/30%；2球赔率{odds['ttg']['s2']}最低，1-1赔率{odds['crs']['1-1']}明显最低，说明市场核心仍是低比分均衡局。法国0-2西班牙暴露了模型对顶级名义客队零封胜的低估：不能只用1-2表达反向结果。因此本场仍主推1-1，但阿根廷0-1/0-2进入核心保护；英格兰2-1/1-0仍是主胜分支，晋级仅小幅领先。比分尾部1-3、0-3和1-4已审计，但因低总进球与双方保守脚本冲突，只有早球叠加追分状态才临场升权。",
        "script":"半场0-0时1-1、1-0、0-0升权；英格兰先球时2-1/1-0升权；阿根廷先球时1-1/1-2/0-1升权；前30分钟出现进球才把3球及2-2抬进主保护层。",
        "odds":f"竞彩HAD {odds['had']['home']}/{odds['had']['draw']}/{odds['had']['away']}；总进球1球 {odds['ttg']['s1']}、2球 {odds['ttg']['s2']}、3球 {odds['ttg']['s3']}；比分1-1 {odds['crs']['1-1']}、2-1 {odds['crs']['2-1']}、1-0 {odds['crs']['1-0']}、0-1 {odds['crs']['0-1']}、0-2 {odds['crs']['0-2']}、1-2 {odds['crs']['1-2']}、0-0 {odds['crs']['0-0']}；半全场胜胜 {odds['hafu']['hh']}、平平 {odds['hafu']['dd']}、平胜 {odds['hafu']['dh']}；竞彩最后更新时间 {daily.get('lastUpdateTime')}。",
        "score_pool":[
            ...
            {"score":"0-1","prob":12,"role":"反向核心","reason":"吸收西班牙零封客胜漏池教训"},
            {"score":"0-2","prob":9,"role":"反向零封尾部","reason":"顶级名义客队领先后控场"},
            {"score":"1-2","prob":9,"role":"爆冷","reason":"阿根廷后程反击或定位球"},
    ...
    raw["prediction"] = {"totalGoals":"2", "scores":["1:1","2:1","1:0","0:1","0:2","1:2","0:0","2:2"], "upset":"0:2", "confidence":"中高", "candidates":["2","1","3"]}
    raw["review"] = "法国0-2复盘校准后：90分钟英格兰轻优、重点防平；2球主线，1-1主比分，阿根廷0-1/0-2进入核心保护，最终晋级英格兰52%。"
    save(daily_path, daily)
    payload102 = {"date":"20260716","stage":"semifinal","matches":[m102],"generatedAt":"2026-07-15 10:59 +08:00","source":"Sporttery live full pools + 101 post-match calibration"}
    save(DATA / "semifinal_prediction_102_20260715.json", payload102)
    ...
    parlay = {
        "date":"20260715","stage":"semifinal_single_match","title":"明日半决赛102预测组合",
        "legs":[
            {"match_no":"101","match":"法国 vs 西班牙","win_draw_loss":"法国胜，防平","total_goals":"2/3球","scores":"2-1 / 1-1 / 1-0","upset":"西班牙1-2","confidence":"中"},
            {"match_no":"102","match":"英格兰 vs 阿根廷","win_draw_loss":"英格兰胜，重点防平","total_goals":"2球，防1/3球","scores":"1-1 / 2-1 / 1-0","upset":"阿根廷0-1 / 0-2 / 1-2","confidence":"中高；竞彩全池已校准"},
        ],
    ...
    save(DATA / "semifinal_parlay_20260715.json", parlay)
    ...
    state["calibration"]["todayAdjustment"] = f"102竞彩{odds['had']['home']}/{odds['had']['draw']}/{odds['had']['away']}与2球/1-1低位：90分钟英格兰轻优、重点防平；主1-1，英格兰2-1/1-0，阿根廷0-1/0-2核心保护，晋级英格兰52%。"
